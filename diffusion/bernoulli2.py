...
class FiLMTime(nn.Module):
    ...
    def forward(self, x, t_emb):
        ...
        """
        gamma_beta = self.mlp(t_emb)  # [B, 2*C]
...
class ConditionalReverseConvModel(nn.Module):
    ...
    def __init__(self, snps, n_channels=256, n_blocks=8, t_emb_dim=128):
        ...
        self.film = FiLMTime(t_emb_dim, n_channels)
    ...
    for epoch in range(epochs):
        ...
        if val_loader and val_mask_indices is not None:
            ...
            if val_loss < best_val_loss:
                ...
                patience_counter = 0
                torch.save(model.state_dict(), "best_model_bern2.pt")
            else:
                patience_counter += 1
...
if __name__ == "__main__":
    ...
    avg_r2 = evaluate_masked_r2_reverse_diffusion(
        ...
        device=device,
        output_path="diff_results_bern2.csv",
        batch_size=batch_size
    )
