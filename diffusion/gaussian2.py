...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, t):
        device = t.device
        half = self.dim // 2
        emb = torch.exp(torch.arange(half, device=device, dtype=torch.float32) * (-math.log(10000) / half))
...
class ConvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size=3, stride=1, padding=1, groups=8):
        ...
        # GroupNorm works better for small batch sizes than BatchNorm1d
        gn = max(1, out_ch // groups)
        self.norm = nn.GroupNorm(gn, out_ch)
        self.act = nn.SiLU()
    def forward(self, x):
...
class ResBlock(nn.Module):
    ...
    def forward(self, x, t_emb):
        ...
        """
        h = self.conv1(x)
        # add timestep conditioning (broadcast over length)
        t = self.t_proj(t_emb).unsqueeze(-1)
        h = h + t
        h = self.conv2(h)
        return (x + h) * (1.0 / math.sqrt(2.0))  # scaled residual

class ConditionalUNet1D(nn.Module):
    """
    1D U-Net conditioned on timestep and mask.
    Input to forward: z_t [B, snps], t [B], mask [B, snps]
    Output: eps_pred [B, snps] (same shape as your current model)
    """
    def __init__(self, snps, base_channels=128, channel_mults=(1,2,4), num_resblocks=2, t_emb_dim=256):
        super().__init__()
        self.snps = snps
        ...
        # timestep embedding
        self.t_pos = SinusoidalPosEmb(t_emb_dim//2 if t_emb_dim % 2 == 0 else t_emb_dim)
        self.t_mlp = nn.Sequential(
            nn.Linear(self.t_pos.dim, t_emb_dim),
            nn.SiLU(),
            nn.Linear(t_emb_dim, t_emb_dim),
        )

        # initial projection: we pass z_t and mask as two channels
        ...
        for mult in channel_mults:
            ...
            # resblocks at this scale
            res_blocks = nn.ModuleList([ResBlock(out_ch, t_emb_dim) for _ in range(num_resblocks)])
            self.down_res.append(res_blocks)
            # downsample conv (stride 2) except if same length desired
            down_conv = nn.Conv1d(in_ch, out_ch, kernel_size=4, stride=2, padding=1)
            self.downs.append(down_conv)
            in_ch = out_ch

        # bottleneck
        self.mid_block1 = ResBlock(in_ch, t_emb_dim)
        self.mid_block2 = ResBlock(in_ch, t_emb_dim)
        ...
        for mult in reversed(channel_mults):
            out_ch = base_channels * mult
            # upsample (ConvTranspose1d stride 2)
            up_conv = nn.ConvTranspose1d(in_ch, out_ch, kernel_size=4, stride=2, padding=1)
            self.ups.append(up_conv)
            res_blocks = nn.ModuleList([ResBlock(out_ch, t_emb_dim) for _ in range(num_resblocks)])
            self.up_res.append(res_blocks)
            in_ch = out_ch

        # final projection to 1 channel (predict noise in logit space)
    ...
    def forward(self, z_t, t, mask):
        """
        z_t: [B, snps]  (logit-space noisy input)
        t: [B]         (timesteps)
        mask: [B, snps] (0/1 observed mask)
        """
        B, S = z_t.shape
        # create timestep embedding
        t_emb = self.t_pos(t)           # [B, t_pos_dim]
        t_emb = self.t_mlp(t_emb)       # [B, t_emb_dim]

        # prepare input channels: z_t and mask -> [B, 2, S]
        x = torch.stack([z_t, mask.float()], dim=1)
        x = self.init_conv(x)           # [B, C, L]

        # store skip connections
        skips = []
        # down path
        idx = 0
        for down_conv, res_blocks in zip(self.downs, self.down_res):
            x = down_conv(x)        # downsample
            for rb in res_blocks:   # then process
                x = rb(x, t_emb)
            skips.append(x)
            idx += 1
        ...
        # up path
        for up_conv, res_blocks in zip(self.ups, self.up_res):
            x = up_conv(x)
            # if shapes don't match due to odd lengths, center-crop/pad
            # ensure skip has same length as x
            skip = skips.pop()
            # ensure skip has same length as x
            if skip.size(-1) != x.size(-1):
                skip_len = skip.size(-1)
                x_len = x.size(-1)
                max_len = max(skip_len, x_len)

                if skip_len < max_len:
                    skip = F.pad(skip, (0, max_len - skip_len))
                if x_len < max_len:
                    x = F.pad(x, (0, max_len - x_len))
            # combine skip
            x = x + skip
            # residual blocks
    ...
    for epoch in range(epochs):
        ...
        if val_loader:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch = epoch + 1
                patience_counter = 0
                torch.save(model.state_dict(), "best_model_gauss2.pt")
            else:
                patience_counter += 1
...
if __name__ == "__main__":
    ...
    reverse_model = ConditionalUNet1D(snps=snps, base_channels=128, channel_mults=(1,2,4), num_resblocks=2, t_emb_dim=256).to(device)
    ...
    reverse_model = ConditionalUNet1D(snps=snps, base_channels=128, channel_mults=(1,2,4), num_resblocks=2, t_emb_dim=256).to(device)  # reinit model
    ...
    avg_r2 = evaluate_masked_r2_reverse_diffusion(
        ...
        device=device,
        output_path="diff_results_gauss2.csv",
        batch_size=batch_size
    )
