    ...
    for epoch in range(epochs):
        ...
        if val_loader:
            ...
            if val_loss < best_val_loss:
                ...
                best_epoch = epoch + 1
                patience_counter = 0
                torch.save(model.state_dict(), "checkpoints/best_model_bern.pt")
            else:
                patience_counter += 1
...
def evaluate_masked_r2_reverse_diffusion(
    ...
    tau_tensor=None,
    n_reverse_runs=5,
    num_sampling_steps=1000  # << fewer timesteps at inference
):
    model.eval()
    ...
    # --- Subsample timestep schedule (like DDIM) ---
    t_schedule = np.linspace(
        diffusion.num_timesteps - 1,  # start from T-1
        0,                            # down to 0
        num_sampling_steps,
        dtype=int
    )
    ...
    for start in tqdm(
        ...
        # --- Multiple reverse runs per batch ---
        for run_idx in tqdm(
            ...
            for t_inv in tqdm(
                ...
                # Deterministic update (posterior mean)
    ...
    logging.info(
        ...
        f"{num_sampling_steps} steps): {avg_r2:.6f}"
    )
...
if __name__ == "__main__":
    ...
    OUTPUT_RESULTS = "results/diff_results_bern.csv"

    # --- Model checkpoints ---
    FINAL_MODEL_PATH = "checkpoints/best_model_bern.pt"
