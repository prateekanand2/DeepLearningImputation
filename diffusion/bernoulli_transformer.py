...
def evaluate_masked_r2_reverse_diffusion(
    ...
    masked_snps = maf_df.iloc[mask_indices].reset_index(drop=True)
    ...
    for start in tqdm(range(0, samples, batch_size), total=num_batches, desc="Batches"):
        ...
        for chunk_start in tqdm(chunk_starts, leave=False, desc="Chunks"):
    ...
    # --- optionally save per-SNP R² with SNP IDs and MAF ---
    if output_path:
...
if __name__ == "__main__":
    ...
    VAL_SPLIT = 0.1
    MASK_RATIO = 0.8
    SKIP_TRAINING = False   # True to skip training and load model
    WINDOW_SIZE = 1024
    STRIDE = 512
    ...
    if not SKIP_TRAINING:
        ...
        wandb.init(
            ...
            name=f"bern_transformer_b38_seed{SEED}",
            config={
                "lr": LR,
                "min_lr": MIN_LR,
                ...
                "stride": STRIDE,
                "model": "ConditionalReverseTransformerModel"
            }
        )
    ...
    logging.info(f"Loaded test matrix: {x_test.shape[0]} samples × {x_test.shape[1]} SNPs")
